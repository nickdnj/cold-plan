import type { Kit, KitId } from '../types';
...
function KitCard({ kit, onSelect }: { kit: Kit; onSelect: () => void }) {
  ...
  return (
    <button
      onClick={onSelect}
      className={`w-full text-left p-5 rounded-2xl border-2 ${colors.border} ${colors.bg} transition-all hover:shadow-lg group`}
    >
...
export function KitCatalog({ onSelectKit, onBack }: Props) {
  return (
    <div className="max-w-2xl mx-auto">
      <h2 className="text-2xl font-bold text-gray-900 mb-2">Cold Plan Kits</h2>
      ...
      <div className="space-y-4 mb-8">
        {kits.map((kit) => (
          <KitCard key={kit.id} kit={kit} onSelect={() => onSelectKit(kit.id)} />
        ))}
      </div>

      <div className="mb-6 p-4 rounded-xl bg-blue-50 border border-blue-200">
        <h3 className="font-semibold text-blue-800 mb-1">Why Kits?</h3>
